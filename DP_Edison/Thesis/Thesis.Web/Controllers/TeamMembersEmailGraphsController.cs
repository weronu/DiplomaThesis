        ...
        public ActionResult FindBrokerage(GraphViewModel graphViewModel)
        {
            try
            {
                ...
                if (response.Succeeded)
                {
                    ...
                    if (graphViewModel.SelectedTeamMemberId == null)
                    {
                        connectionString = _importConnectionString;
                        graphViewModel.FileImported = true;
                    }
                    else
                    {
                        connectionString = GetConnectionStringBasedOnSelectedMember(graphViewModel.SelectedTeamMemberId.ToString());
                    }
                    FetchListServiceResponse<BrokerageDto> topTenBrokersResponse = _graphService.FetchTopTenBrokers(graphViewModel.Graph, connectionString);
